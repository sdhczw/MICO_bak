...
OSStatus MICOStartApplication( mico_Context_t * const inContext )
{
  ...
  require_noerr_action( err, exit, app_log("ERROR: Unable to start the uart recv thread.") );
	...
	err = mico_rtos_create_thread(NULL, MICO_APPLICATION_PRIORITY, "HF_Cloudfunc", MX_Cloudfunc, STACK_SIZE_LOCAL_TCP_SERVER_THREAD, (void*)inContext );
	require_noerr_action( err, exit, app_log("ERROR: Unable to start the MX_Cloudfunc thread.") );



	err = mico_rtos_create_thread(NULL, MICO_APPLICATION_PRIORITY, "HF_CloudRecvfunc", MX_CloudRecvfunc, STACK_SIZE_REMOTE_TCP_CLIENT_THREAD, (void*)inContext );
	require_noerr_action( err, exit, app_log("ERROR: Unable to start the MX_CloudRecvfunc thread.") );
exit:
  return err;
}
